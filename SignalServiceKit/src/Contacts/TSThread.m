...
//  }
//}
- (NSUInteger)unReadMessagesWithTransaction:(YapDatabaseReadTransaction *)transaction
{
    NSMutableArray<id<OWSReadTracking>> *messages = [NSMutableArray new];
    OWSAssert(transaction);

    id result = [transaction ext:TSUnreadDatabaseViewExtensionName];

   
    
    [result enumerateRowsInGroup:self.uniqueId
                  usingBlock:^(
                      NSString *collection, NSString *key, id object, id metadata, NSUInteger index, BOOL *stop) {
...
- (nullable NSString *)atPersonsInteractionWithTransaction:(YapDatabaseReadTransaction *)transaction
{
    OWSAssert(transaction);
//TSUnreadDatabaseViewExtensionName TSMessageDatabaseViewExtensionName
//    __block NSUInteger missedCount = 0;
    __block NSMutableString *atPersonsString = nil;
...
//         missedCount++;
         ...
         if ([TSThread shouldInteractionAppearAtPersons:interaction] && [interaction isKindOfClass:[TSIncomingMessage class]]) {
             TSIncomingMessage *theAtPersons = interaction;
             if (theAtPersons.atPersons &&theAtPersons.atPersons.length>0) {
                 //todo 通过view分离unreadview && at了某人 需要优化为精确at本人信息，但业务耦合需要分析。
                 if (!atPersonsString) {
                     atPersonsString = [[NSMutableString alloc] initWithCapacity:20];
                 }
                 [atPersonsString appendString:theAtPersons.atPersons];
             }else{
...
- (NSString *)atPersonsWithTransaction:(YapDatabaseReadTransaction *)transaction{
