...
public class SignalAttachment: NSObject {
    ...
    public var filenameOrDefault: String {
        if let filename = sourceFilename {
            ...
            let kDefaultAttachmentName = "Chative"
    ...
    public class func compressVideoAsMp4(dataSource: DataSource, dataUTI: String) -> (Promise<SignalAttachment>, AVAssetExportSession?) {
        ...
        Logger.debug("\(String(describing: self.logTag)) generate export mp4 url: \(exportURL)")
        ...
        exportSession.exportAsynchronously {
            ...
            Logger.debug("\(self.logTag) sourceFilename : \(dataSource.sourceFilename)")
            Logger.debug("\(self.logTag) mp4Filename : \(mp4Filename)")
            
            if exportSession.status == AVAssetExportSessionStatus.completed {
                Logger.debug("convert success.")
            } else {
                Logger.debug("convert failed, \(exportSession.error)")
            }

            guard let newDataSource = DataSourcePath.dataSource(with: exportURL) else {
                NSLog("Failed to build data source for exported video URL")
                let attachment = SignalAttachment(dataSource: DataSourceValue.emptyDataSource(), dataUTI: dataUTI)
                attachment.error = .couldNotConvertToMpeg4
                fulfill(attachment)
                return
            ...
            newDataSource.setShouldDeleteOnDeallocation()
            newDataSource.sourceFilename = mp4Filename

            let attachment = SignalAttachment(dataSource: newDataSource, dataUTI: kUTTypeMPEG4 as String)
            fulfill(attachment)
        }
