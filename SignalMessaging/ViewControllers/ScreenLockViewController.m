...
- (void)updateUIWithState:(ScreenLockUIState)uiState isLogoAtTop:(BOOL)isLogoAtTop animated:(BOOL)animated
{
    ...
    if (uiState == ScreenLockUIStateOffline) {
        if (TSAccountManager.sharedInstance.isDeregistered) {
            self.screenBlockingLabel.text = NSLocalizedString(@"NETWORK_STATUS_DEREGISTERED",
            ...
            switch ([TSSocketManager sharedManager].state) {
                case SocketManagerStateClosed:
                    self.screenBlockingLabel.text = NSLocalizedString(@"NETWORK_STATUS_OFFLINE", @"");
                    self.screenBlockingLabel.textColor = [UIColor ows_redColor];
                    break;
                case SocketManagerStateConnecting:
                    self.screenBlockingLabel.text = NSLocalizedString(@"NETWORK_STATUS_CONNECTING", @"");
                    self.screenBlockingLabel.textColor = [UIColor ows_yellowColor];
                    break;
                case SocketManagerStateOpen:
                    self.screenBlockingLabel.text = NSLocalizedString(@"NETWORK_STATUS_CONNECTED", @"");
