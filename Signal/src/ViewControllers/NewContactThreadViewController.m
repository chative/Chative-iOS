...
//                                                      action:@selector(dismissPressed)];
    ...
    if (isLoadInternalContactsOver) {
        ...
        // fetch internal contacts when the page is showed first time.
        [self.tableViewController.tableView setContentOffset:CGPointMake(0, -44) animated:YES];
        [pullToRefreshView sendActionsForControlEvents:UIControlEventValueChanged];
...
// modified: hide inviting friend from system contacts.
...
- (NSArray<OWSTableSection *> *)contactsSectionsForSearch
{
    ...
    for (NSString *phoneNumber in searchPhoneNumbers) {
        ...
        // modified: diable SMS invitation.
//        else {
...
//                                               action:ConversationViewActionCompose];
    ...
    [viewController configureForThread:thread action:ConversationViewActionCompose focusMessageId:nil];
