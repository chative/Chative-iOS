...
- (void)setAudioIcon:(UIImage *)icon
{
    ...
    _audioPlayPauseButton.layer.borderWidth = 1;
    _audioPlayPauseButton.layer.borderColor = [[UIColor ows_dark30Color] CGColor];
}
...
- (void)updateAudioProgressView
{
    ...
    UIColor *progressColor = self.isIncoming ? [UIColor ows_light60Color] : [UIColor ows_signalBlueColor];
...
- (void)createContents
{
    ...
    bgView.backgroundColor = [UIColor ows_whiteColor];
    ...
    [self updateAudioBottomLabel];
    bottomLabel.textColor = self.isIncoming ? [UIColor ows_blackColor] : [UIColor ows_signalBlueColor];
